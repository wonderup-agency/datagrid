...
(() => {
  const component = document.querySelector("[data-component='pricing']");
  if (!component) return;
  ...
  const expandFeaturedCard = () => {
    if (window.innerWidth > 992) {
      ...
      featuredCard.style.height =
        featuredCard.offsetHeight + totalHeight + "px";
    } else {
      // cleanup if <992
      const featuredCard = component.querySelector(
        '[data-wf--nested-pricing-card-variant--variant="visible"]',
  ...
  cards.forEach((card) => {
    ...
    const costPerMonthEl = card.querySelector(
      "[data-pricing='cost-per-month']",
    );
